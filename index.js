 ...
 * Usage is as follows
 * open gif: gif()
 * open gif with config: gif({..})
 * close gif, no print, throw error: gif(false | 0)
 * close gif, print and throw error: gif('log')
 * close gif, not throw err: gif('ignore')
 */
...
export default function(arg= {}) {
    ...
    let gif= $('#'+ gif_id);
    if(!gif.length){
        let config= argType === 'object'? $.extend({}, defaultConfig, arg): defaultConfig;
        gif= $(`
            <div id="${gif_id}" style="position:fixed;bottom:50%;right:50%;width:${config.width}px;height:${config.height}px;z-index:10000;visibility:hidden">
